...
exports.usaSpendingService = void 0;
// =============================================================
...
class UsaSpendingService {
    constructor() {
        this.client = axios_1.default.create({
            baseURL: config_1.config.usaSpending.baseUrl,
            timeout: 30000,
    ...
    async enrichOpportunity(params) {
        try {
            ...
            logger_1.logger.warn('USASpending enrichment failed', {
     ...
     */
    async getAgencySetAsideRates(agencyName) {
        try {
            const endDate = new Date().toISOString().split('T')[0];
            ...
            const sbTotal = sbResponse.status === 'fulfilled'
                ? sbResponse.value.data?.page_metadata?.total || 0
                : 0;
            const sdvosbTotal = sdvosbResponse.status === 'fulfilled'
                ? sdvosbResponse.value.data?.page_metadata?.total || 0
                : 0;
            const grandTotal = totalResponse.status === 'fulfilled'
     ...
     */
    computeAgencyAlignmentScore(stats, isSdvosb) {
        if (!stats)
            return 0.5;
        let score = 0.5;
        ...
        if (stats.totalAwards > 1000)
            score += 0.1;
        else if (stats.totalAwards > 100)
            score += 0.05;
        return Math.min(score, 1.0);
    }
}
exports.usaSpendingService = new UsaSpendingService();
