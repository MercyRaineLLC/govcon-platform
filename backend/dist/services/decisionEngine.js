...
async function evaluateBidDecision(opportunityId, clientCompanyId) {
    ...
    const naicsMatch = client.naicsCodes.some((code) => code.trim() === opportunity.naicsCode.trim());
    if (!naicsMatch) {
    ...
    const probabilityResult = (0, probabilityEngine_1.scoreOpportunityForClient)({
    ...
    triggeredFlags.push(`8-factor model: raw=${probabilityResult.rawScore.toFixed(3)}, ` +
    ...
    if (opportunity.recompeteFlag && opportunity.incumbentProbability !== null) {
        if (opportunity.incumbentProbability < 0.5) {
            ...
            triggeredFlags.push(`Recompete with strong incumbent (${(opportunity.incumbentProbability * 100).toFixed(0)}% win rate)`);
        }
    }
    if (opportunity.historicalAvgAward && estValue) {
        const sizeRatio = estValue / Number(opportunity.historicalAvgAward);
        if (sizeRatio > 2.0) {
            triggeredFlags.push(`Contract value ${sizeRatio.toFixed(1)}x historical avg — potential scope expansion`);
            riskScore += 5;
        }
        else if (sizeRatio < 0.5) {
            triggeredFlags.push(`Contract value ${sizeRatio.toFixed(1)}x historical avg — potential scope reduction`);
        }
    }
    ...
    if (stats && (stats.totalWon + stats.totalLost) > 0) {
        ...
        triggeredFlags.push(`Bayesian calibrated: ${stats.totalWon}W/${stats.totalLost}L ` +
    ...
    if (complianceBlocked) {
        ...
        triggeredFlags.push(`Strong opportunity: ROI ${roiRatio.toFixed(1)}x, probability ${(winProbability * 100).toFixed(0)}%`);
    }
    else if (roiRatio > 1.5 && winProbability > 0.2) {
        recommendation = 'BID_PRIME';
        triggeredFlags.push(`Moderate-strong opportunity: ROI ${roiRatio.toFixed(1)}x, probability ${(winProbability * 100).toFixed(0)}%`);
    }
    else if (winProbability > 0.2) {
        recommendation = 'BID_SUB';
        triggeredFlags.push(`Sub-contract opportunity: probability ${(winProbability * 100).toFixed(0)}%`);
    }
    else {
        triggeredFlags.push(`Below threshold: ROI ${roiRatio.toFixed(1)}x, probability ${(winProbability * 100).toFixed(0)}%`);
    }
    // ------------------------------------------------------------
    // 7. Persist Decision
    // ------------------------------------------------------------
    ...
    logger_1.logger.info('Bid decision evaluated', {
        opportunityId,
        clientCompanyId,
        recommendation,
