    ...
    try {
        ...
        const clients = await database_1.prisma.clientCompany.findMany({
            where: clientCompanyId
                ? { id: clientCompanyId, consultingFirmId }
                : { consultingFirmId },
            select: { id: true },
        });
...
router.post('/', async (req, res, next) => {
    try {
        ...
        if (!rewardType)
            throw new errors_1.ValidationError('rewardType required');
        if (!description)
            throw new errors_1.ValidationError('description required');
        if (!triggerReason)
            throw new errors_1.ValidationError('triggerReason required');
        const client = await database_1.prisma.clientCompany.findFirst({ where: { id: clientCompanyId, consultingFirmId } });
        if (!client)
            throw new errors_1.NotFoundError('Client not found');
        const reward = await database_1.prisma.complianceReward.create({
            data: {
                clientCompanyId,
                rewardType,
                description,
        ...
        logger_1.logger.info('Compliance reward granted', { rewardId: reward.id, clientCompanyId });
...
router.post('/evaluate/:clientCompanyId', async (req, res, next) => {
    try {
        const consultingFirmId = (0, tenant_1.getTenantId)(req);
        ...
        const existing = await database_1.prisma.complianceReward.findMany({
            where: { clientCompanyId },
            select: { triggerReason: true },
        });
        const alreadyGranted = new Set(existing.map((r) => r.triggerReason));
        const granted = [];
        // Check FIRST_ONTIME: first on-time submission
        if (!alreadyGranted.has('FIRST_ONTIME')) {
            const hasOnTime = submissions.some((s) => s.wasOnTime);
            if (hasOnTime) {
                const reward = await database_1.prisma.complianceReward.create({
                    data: {
                        clientCompanyId,
                ...
                granted.push(reward);
                logger_1.logger.info('FIRST_ONTIME reward granted', { clientCompanyId });
            }
        }
        ...
        if (!alreadyGranted.has('5_CONSECUTIVE_ONTIME') && submissions.length >= 5) {
            ...
            if (last5.every((s) => s.wasOnTime)) {
                ...
                granted.push(reward);
                logger_1.logger.info('5_CONSECUTIVE_ONTIME reward granted', { clientCompanyId });
            }
        }
        ...
        if (!alreadyGranted.has('PERFECT_COMPLIANCE') && submissions.length >= 10) {
            const onTimeCount = submissions.filter((s) => s.wasOnTime).length;
            if (onTimeCount === submissions.length) {
                ...
                granted.push(reward);
                logger_1.logger.info('PERFECT_COMPLIANCE reward granted', { clientCompanyId });
            }
        }
...
router.put('/:id/redeem', async (req, res, next) => {
    try {
        const consultingFirmId = (0, tenant_1.getTenantId)(req);
        const reward = await database_1.prisma.complianceReward.findUnique({ where: { id: req.params.id } });
        if (!reward)
            throw new errors_1.NotFoundError('Reward not found');
        // Verify belongs to this firm
        const client = await database_1.prisma.clientCompany.findFirst({ where: { id: reward.clientCompanyId, consultingFirmId } });
        if (!client)
            throw new errors_1.NotFoundError('Reward not found');
        const updated = await database_1.prisma.complianceReward.update({
