...
function generateClientToken(payload) {
    return jsonwebtoken_1.default.sign(payload, config_1.config.jwt.secret, { expiresIn: '24h' });
}
function authenticateClientJWT(req, res, next) {
    ...
    try {
        ...
        if (payload.role !== 'CLIENT')
            throw new errors_1.UnauthorizedError('Not a client token');
...
router.post('/auth/register', async (req, res, next) => {
    try {
        ...
        const user = await database_1.prisma.clientPortalUser.create({
        ...
        logger_1.logger.info('Client portal user created', { id: user.id, clientCompanyId });
...
router.post('/auth/login', async (req, res, next) => {
    try {
        ...
        if (!email || !password)
            throw new errors_1.ValidationError('email and password required');
        const user = await database_1.prisma.clientPortalUser.findUnique({ where: { email } });
        if (!user || !user.isActive)
            throw new errors_1.UnauthorizedError('Invalid credentials');
        const valid = await bcryptjs_1.default.compare(password, user.passwordHash);
        if (!valid)
            throw new errors_1.UnauthorizedError('Invalid credentials');
        await database_1.prisma.clientPortalUser.update({
            where: { id: user.id },
        ...
        const clientCompany = await database_1.prisma.clientCompany.findUnique({
...
router.get('/dashboard', authenticateClientJWT, async (req, res, next) => {
    try {
        ...
        const [docRequirements, penalties, rewards, bidDecisions, client] = await Promise.all([
            ...
            database_1.prisma.complianceReward.findMany({
            ...
            database_1.prisma.clientCompany.findUnique({
                where: { id: clientCompanyId },
                include: { performanceStats: true },
            }),
        ]);
        const now = new Date();
        const enrichedRequirements = docRequirements.map((r) => {
        ...
        res.json({
            ...
            data: {
                ...
                summary: {
                    ...
                    submitted: docRequirements.filter((r) => r.status === 'SUBMITTED').length,
                    pending: docRequirements.filter((r) => r.status === 'PENDING').length,
                    overdue: enrichedRequirements.filter((r) => r.urgency === 'OVERDUE').length,
...
// -------------------------------------------------------------
router.get('/score-breakdown/:opportunityId', authenticateClientJWT, async (req, res, next) => {
    try {
        const { clientCompanyId } = req.clientUser;
        const bidDecision = await database_1.prisma.bidDecision.findFirst({
        ...
        if (!bidDecision)
            throw new errors_1.NotFoundError('Score breakdown not found for this opportunity');
        const breakdown = bidDecision.opportunity.scoreBreakdown;
        // Build plain-language explanations per factor
        const plainExplanations = breakdown?.factorContributions?.map((f) => ({
            ...f,
...
// -------------------------------------------------------------
router.get('/rewards', authenticateClientJWT, async (req, res, next) => {
    try {
        const { clientCompanyId } = req.clientUser;
        const rewards = await database_1.prisma.complianceReward.findMany({
            where: { clientCompanyId },
            orderBy: { createdAt: 'desc' },
        });
...
// Helpers
// -------------------------------------------------------------
function buildPlainExplanation(factor, score) {
    ...
    return (map[factor] ?? ((p) => `Score: ${p}%`))(pct);
}
function buildSummary(probability) {
    const pct = Math.round(probability * 100);
    if (pct >= 65)
        return `Strong fit — our model gives you a ${pct}% chance of winning this contract. You should seriously consider bidding.`;
    if (pct >= 45)
        return `Moderate fit — our model gives you a ${pct}% chance. This is competitive but winnable with a strong proposal.`;
    if (pct >= 25)
        return `Challenging — our model gives you a ${pct}% chance. Consider carefully whether the investment is worthwhile.`;
